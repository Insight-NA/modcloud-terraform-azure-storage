...
This module was defaults to a Standard StorageV2, with a Hot access tier, and read-access geo-redunant replication. For Premium accounts, account replication must be either locally-redundant (LRS) or zone-redundant (ZRS). A storage account name can be specified, or autogenerated in the format of ```st<app code><app environment><app instance><random identifer>```; if manually specifing the name, remember storage account names must be between 3 and 24 characters, lowercase letters and numbers, and globally unique.
...
For Azure Cross-Region Replication, East US 2 is paired wtih Central US.
...
azure-storage-account-https-only-traffic-true.sentinel - enable https traffic only set to true
azure-storage-account-infrastructure-encryption-true.sentinel - infrastructure encryption enabled set to true
azure-storage-account-min-tls-version-12-require.sentinel - miniumim TLS version is set to highest available, TLS1.2
azure-storage-account-nested-items-public-false.sentinel - allow nested items to be public is set to false
azure-storage-account-network-rules-default-action-deny-require.sentinel - default action is DENY
azure-storage-account-publish-internet-endpoints-false.sentinel - publish internet endpoints is false
azure-storage-account-logging-retention-days-require.sentinel - retention policy days must be greater than zero (NOTE: because of current API implmenation, this can only be set for Queues)
azure-storage-account-minute-metrics-retention-days-require.sentinel - retention policy days must be greater than zero (NOTE: because of current API implmenation, this can only be set for Queues)
...
| <a name="input_storage_account_name"></a> [storage\_account\_name](#input\_storage\_account\_name) | (Required) Specifies the name of the storage account. Only lowercase Alphanumeric characters allowed. Changing this forces a new resource to be created. This must be unique across the entire Azure service, not just within the resource group. | `string` | `null` | no |
